...
def plot_noisy_signal(measures, save=False, fname=None):
    ...
    noise_var = measures[:,0] ** 2
    potential_mean = measures[:,1]
    potential_std = measures[:,2]
    plt.figure(1)
    plt.fill_between(noise_var, potential_mean-2*potential_std, potential_mean+2*potential_std)
    plt.plot(noise_var, potential_mean)
    plt.xlabel('Noise variance', fontsize=18)
    plt.ylabel('Potential', fontsize=18)
    plt.tick_params(axis='both', which='major', labelsize=11)
    if save: plt.savefig(fname)
    plt.show()
